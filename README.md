...
- Temporary notification pauses from the tray or Settings for 30 minutes, 1 hour, 2 hours, 4 hours, or until the following morning, with an immediate resume action. Suppressed alerts are not replayed on resume, and problems first detected during a pause do not produce recovery messages.
- Automatic refresh of every enabled provider every 1, 5, 15, or 30 minutes, including while hidden in the notification area, with manual refresh at any time.
- Failed usage checks receive up to two confirmation checks about a minute apart before returning to the normal schedule. Provider-requested waiting periods take priority, including for manual refreshes.
- After a temporary refresh failure, new limit warnings can still appear when fresh data confirms the same usage cycle. Paused alerts are not replayed.
...
- Claude reads the authenticated `/usage` view through Claude Code itself, so sign-in stays with Claude Code. With Claude Code 2.1.178 or later, UsageDeck runs `claude -p /usage`, which Claude Code answers locally without a model request, with tools and MCP servers disabled and no session saved. Older versions open `/usage` in an isolated terminal session with tools disabled, and UsageDeck answers Claude Code's one-time folder trust prompt for its folder only. Both run in UsageDeck's own empty `ClaudeProbe` folder. UsageDeck reads the plan name from Claude Code's local sign-in file and never writes to that file. **Settings → Providers → Claude → Faster direct usage check** is off by default. When you turn it on, UsageDeck sends the access token stored by Claude Code only to Anthropic's fixed usage endpoint and falls back to the `/usage` view if that fails. The direct check also shows limit resets that Claude Code offers through `/limit-reset`; Anthropic reports these only to a current Claude Code CLI, so this request identifies itself with the installed Claude Code version. Anthropic does not document that endpoint or officially support its use by other apps, and UsageDeck asks you to confirm the risk before enabling it.
